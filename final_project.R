...
#Load data
data <- read_delim("FINAL PROJECT - data.csv")
...
linear_plot <- plot (table$monthly_listeners, table$year_of_song)
print(linear_plot)

# add x line and y line for means
meany <- mean(table$monthly_listeners)
meanx <- mean(table$year_of_song)
...
linear_relationship <- lm(monthly_listeners ~ year_of_song, data = table)
...
##### STEP 3: Plot the music

# Plot the music
plot(table$monthly_listeners, music(linear_relationship))
